...
export async function getExcelDemo2() {
  ...
  // 垂直左右居中加上自动换行
  const content_row_item = { vertical: 'middle', horizontal: 'center', wrapText: true }
  ...
  const C9 = sheet.getCell('C9')
  C9.alignment = content_row
  C9.font = table_header2
  C9.fill = table_fill3

  sheet.mergeCells('E9:F9')
  const E9 = sheet.getCell('E9')
  ...
  // 第15到第15+n-1行
  const n = 5 // n是配置信息或者通用软件信息中的信息最大条数
  const cols = 'CDEFGHIJ'
  const row_list = [] // 存放第15到第15+n行的单元格
  for (let i = 1; i <= n; i++) {
    sheet.getRow(14 + i).height = 15.25
    const item_row = []
    for (const col of cols) {
      const cell = sheet.getCell(col + (14 + i))
  ...
  for (const i of 'CG') {
    const item_cell = sheet.getCell(i + num)
  ...
  // 第27 +n-1到27 +n-1 + x-1行
  num = 27 + n - 1
  const x = 1 + 2 // x是专用软件信息中信息最大条数 第一行是表头
  const C27 = []
  ...
  for (const i in tables) {
    const item_cell = sheet.getCell(tables[i] + num)
  ...
  num = 31 + n - 1 + x - 1
  const z = 4 // z是系统用户信息中信息的最大条数
  const C31 = []
  for (let item = 0; item < z; item++) {
    sheet.getRow(num).height = 15
    sheet.mergeCells('E' + num + ':' + 'F' + num)
    const item_list = []
    for (const i of 'CDEGHIJ') {
      const item_cell = sheet.getCell(i + num)
      item_cell.alignment = content_row
      item_cell.font = table_header3
      item_cell.fill = table_fill5
  ...
  const C48 = []
  const y = 55 // y是’链接（服务）用户信息’中的最大条数
  let c3 = 0 // 表格超出数
  if (7 + c + 2 + c2 < y) {
    c3 = y - (7 + c + 2 + c2)
  ...
  for (const item of [C27, C31, C39, C44]) {
    for (const i of item) {
      r.push(i[i.length - 1])
    }
  }
  ...
  if (C48.length > 0) {
    ...
    for (const item in C48[C48.length - 1]) {
      if (item > 0 || item < 4) {
    ...
    for (const item in C47[C47.length - 1]) {
      console.log(item)
      if (item > 0 || item < 4) {
